...
Available only when using the compatibility profile.
      ...
      It is intended to be used to compute *gl_Position*, e.g.
...
Use the texture coordinate *P* to do a texture lookup in the texture
      ...
      *Dref* and the array layer comes from the second to last component
      of *P*.
...
undefined.
However, **anyInvocation**() is guaranteed to return **true** if *value* is
**true**, and **allInvocations**() is guaranteed to return **false** if *value* is
**false**.
