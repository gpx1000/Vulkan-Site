...
- **Component**: features
- **Version**: latest
- **URL**: /features/latest/features/proposals/VK_KHR_shader_integer_dot_product.html
...
- [3.2. API Properties](#_api_properties)
- [3.2._API_Properties](#_api_properties)
...
The question then becomes *which* dedicated dot product operations should be exposed if there is no common subset of accelerated operations.
Choices become:
...
Newer backends such as those using [MLIR](https://www.tensorflow.org/mlir) are looking at generating platform-specific optimized IR, which can be done in part by expressing the macro-level operations differently.
...
Properties are written as `integerDotProduct{type bitwidth}{Unsigned|Signed|MixedSignedness}Accelerated`.
Each property corresponds to a SPIR-V opcode of the form `Op{U|S|SU}DotKHR`, as defined in SPIR-V extension SPV_KHR_integer_dot_product.
The `` portion of the property corresponds to the `AccSat` instruction variants.
