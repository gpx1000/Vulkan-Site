...
- **Component**: features
- **Version**: latest
- **URL**: /features/latest/features/proposals/VK_EXT_mutable_descriptor_type.html
...
- [3.1. Mutable Descriptor Type](#_mutable_descriptor_type)
- [3.1._Mutable_Descriptor_Type](#_mutable_descriptor_type)
- [3.1.1. Supported descriptor types](#_supported_descriptor_types)
- [3.1.1._Supported_descriptor_types](#_supported_descriptor_types)
- [3.1.2. Performance considerations](#_performance_considerations)
- [3.1.2._Performance_considerations](#_performance_considerations)
- [3.2. Host-Only Descriptor Sets](#_host_only_descriptor_sets)
- [3.2._Host-Only_Descriptor_Sets](#_host_only_descriptor_sets)
- [3.3. Features](#_features)
- [4. Examples](#_examples)
- [4.1. Specifying a descriptor binding equivalent to a DirectX 12 CBV_SRV_UAV heap](#_specifying_a_descriptor_binding_equivalent_to_a_directx_12_cbv_srv_uav_heap)
- [4.1._Specifying_a_descriptor_binding_equivalent_to_a_DirectX_12_CBV_SRV_UAV_heap](#_specifying_a_descriptor_binding_equivalent_to_a_directx_12_cbv_srv_uav_heap)
- [4.2. Accessing a mutable descriptor in a shader](#_accessing_a_mutable_descriptor_in_a_shader)
- [4.2._Accessing_a_mutable_descriptor_in_a_shader](#_accessing_a_mutable_descriptor_in_a_shader)
- [4.2.1. GLSL](#_glsl)
- [4.2.2. HLSL](#_hlsl)
...
Firstly, solving this in external software has been attempted (notably by [vkd3d](https://github.com/ValveSoftware/vkd3d)) and no satisfying options could be identified; there are workarounds but they are either too slow or too memory intensive to emulate DirectX 12 content at native performance.
...
Typically when specifying a [descriptor set layout binding](https://docs.vulkan.org/spec/latest/chapters/descriptorsets.html#VkDescriptorSetLayoutBinding), applications have to choose one of the available [descriptor types](https://docs.vulkan.org/spec/latest/chapters/descriptorsets.html#VkDescriptorType) that will occupy that binding.
...
`VkMutableDescriptorTypeCreateInfoEXT` can be added to the `pNext` chain of [VkDescriptorSetLayoutCreateInfo](https://docs.vulkan.org/spec/latest/chapters/descriptorsets.html#VkDescriptorSetLayoutCreateInfo), where each entry in `pMutableDescriptorTypeLists` corresponds to a binding at the same index in `pBindings`.
...
Host-only descriptor sets cannot be bound to a command buffer, and their contents must be copied to a non-host-only set using [vkUpdateDescriptorSets](https://docs.vulkan.org/spec/latest/chapters/descriptorsets.html#vkUpdateDescriptorSets) before those descriptors can be used.

Descriptor pools are specified as host-only using a new [create flag](https://docs.vulkan.org/spec/latest/chapters/descriptorsets.html#VkDescriptorSetLayoutCreateFlagBits):

VK_DESCRIPTOR_POOL_CREATE_HOST_ONLY_BIT_EXT = 0x00000004

Any descriptor set created from a pool with this flag set is a host-only descriptor set.

The memory layout of a descriptor set may also be optimized for device access rather than host access, so a new [create flag](https://docs.vulkan.org/spec/latest/chapters/descriptorsets.html#VkDescriptorSetLayoutCreateFlagBits) is provided to specify when a layout will be used with a host-only pool:
...
In addition, as such layouts are not valid for device access, [VkPipelineLayout](https://docs.vulkan.org/spec/latest/chapters/descriptorsets.html#VkPipelineLayout) objects cannot be created with such descriptor set layouts.
...
DirectX 12 descriptor heaps can be specified for general resources containing all types of buffer and image descriptors using the [D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV](https://docs.microsoft.com/en-us/windows/win32/api/d3d12/ne-d3d12-d3d12_descriptor_heap_type) type.
...
* 
[dxil-spirv](https://github.com/HansKristian-Work/dxil-spirv) implements this.
