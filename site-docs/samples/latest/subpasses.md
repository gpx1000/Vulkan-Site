...
- **Component**: samples
- **Version**: latest
- **URL**: /samples/latest/samples/performance/subpasses/README.html
...
- [Merging](#_merging)
- [Transient attachments](#_transient_attachments)
...
The *Subpasses sample* implements a [deferred renderer](https://en.wikipedia.org/wiki/Deferred_shading), which splits rendering in two passes:

* 
A geometry pass, where data needed for shading is written to the *G-buffer* (depth, albedo, normals).
...
![Render passes](../../../_images/samples/performance/subpasses/images/render-passes.jpg)
...
Another technique uses a *single* render pass which is composed of two subpasses.
...
![Subpasses vs render passes trace](../../../_images/samples/performance/subpasses/images/subpasses-renderpasses-trace.jpg)
...
* 
If the number of unique `VkAttachments` used for input and color attachments in all considered subpasses is 

On Mali devices in order to be merged, subpasses are required to use at most 128-bit per pixel of *tile buffer color storage*, although some of the more recent GPUs such as Mali-G72 increase this to up to 256-bits per pixel.
...
![Good practice](../../../_images/samples/performance/subpasses/images/good-practice.jpg)
...
![G-buffer size](../../../_images/samples/performance/subpasses/images/gbuffer-size.jpg)

Some framebuffer attachments in the sample (such as *depth*, *albedo*, and *normal*), are cleared at the beginning of the render pass, written by the geometry subpass, read by the lighting subpass, and discarded at the end of the render pass.
...
[Getting World Position from Depth Buffer Value](https://stackoverflow.com/a/32246825) - stackoverflow.com

[Render Pass](https://vulkan.lunarg.com/doc/view/1.0.37.0/linux/vkspec.chunked/ch07.html) - vulkan.lunarg.com
...
* 
Follow the basic [render pass best practices](../render_passes/README.html), with `LOAD_OP_CLEAR` or `LOAD_OP_DONT_CARE` for attachment loads and `STORE_OP_DONT_CARE` for transient stores.
