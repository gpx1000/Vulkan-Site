...
- **Component**: samples
- **Version**: latest
- **URL**: /samples/latest/samples/performance/constant_data/README.html

## Table of Contents

- [Overview](#_overview)
- [What is constant data?](#_what_is_constant_data)
- [What_is_constant_data?](#_what_is_constant_data)
- [Introduction](#_introduction)
- [Constant data in Vulkan shaders](#_constant_data_in_vulkan_shaders)
- [Constant_data_in_Vulkan_shaders](#_constant_data_in_vulkan_shaders)
- [Varying Types](#_varying_types)
- [Uniform Types](#_uniform_types)
- [Interface Blocks](#_interface_blocks)
- [Vulkan API](#_vulkan_api)
- [The Methods](#_the_methods)
- [Sample Overview](#_sample_overview)
...
- [Buffer Object](#_buffer_object)
- [Performance](#_performance_2)
- [Dynamic Descriptor Sets](#_dynamic_descriptor_sets)
- [Dynamic_Descriptor_Sets](#_dynamic_descriptor_sets)
- [Introduction](#_introduction_5)
- [Performance](#_performance_3)
- [Update-after-bind Descriptor Sets](#_update_after_bind_descriptor_sets)
- [Update-after-bind_Descriptor_Sets](#_update_after_bind_descriptor_sets)
- [Introduction](#_introduction_6)
- [Performance](#_performance_4)
- [Buffer Object Arrays](#_buffer_object_arrays)
- [Buffer_Object_Arrays](#_buffer_object_arrays)
...
input vertex data) in the sense that it remains *constant* across every shader invocation of a draw call.
...
They are *read-only* buffers, so trying to edit them in shader code will result in a compile-time error.
...
They work exactly the same way as a `struct` in GLSL/C++.
For example, to access the model matrix in this interface block, you’d use `per_mesh_data.model_matrix`.

You can read more about interface blocks [here](https://www.khronos.org/opengl/wiki/Type_Qualifier_(GLSL)#Shader_stage_inputs_and_outputs).
...
* 
[Vulkan Tutorial vertex buffer creation](https://docs.vulkan.org/tutorial/latest/04_Vertex_buffers/01_Vertex_buffer_creation.html)

* 
[Vulkan tutorial abstracting buffer creation](https://docs.vulkan.org/tutorial/latest/04_Vertex_buffers/02_Staging_buffer.html#_abstracting_buffer_creation)

* 
[Vulkan tutorial uniform buffer](https://docs.vulkan.org/tutorial/latest/05_Uniform_buffers/00_Descriptor_set_layout_and_buffer.html#_uniform_buffer)
...
![graph](../../../_images/samples/performance/constant_data/images/graph_data.jpg)
...
![sample](../../../_images/samples/performance/constant_data/images/controls.png)
...
So that the shader can understand where this data will be sent, we specify a special push constants `` in our shader code.
...
![push constants](../../../_images/samples/performance/constant_data/images/push_constants_performance.jpg)

In early implementations of Vulkan on Arm Mali, this was usually the fastest way of pushing data to your shaders.
In more recent times, we have observed on Mali devices that *overall* they can be slower.
...
These **descriptors** are simple structures holding a pointer to the resource it is "describing", along with an associated *resource binding* so that when we execute a draw call the shader knows where to look for the resource.
...
The `set` value maps to the *set binding*, and the `binding` value maps to the *resource binding*.
...
These map to the shader `` type `uniform`s and `buffer`s respectively.

![single ubo](../../../_images/samples/performance/constant_data/images/descriptor_set_performance.jpg)
...
Dynamic descriptor sets differ to the regular descriptor sets because they allow an offset to be specified when we are *binding* (`vkCmdBindDescriptorSets`) the descriptor set.
...
In our draw call, for each mesh, *dynamically* offset into the giant **uniform buffer object**.

![dynamic ubo](../../../_images/samples/performance/constant_data/images/dynamic_descriptor_set_performance.jpg)

In the screenshot above (taken on an S10 with a Mali G76 GPU) we can see the load/store cycles stay roughly the same compared to [static uniform buffer objects](#uniform-buffer-objects).
...
This is the concept of allocating all of your constant data *upfront* in a large buffer, and writing the entire buffer to a descriptor set.
...
**Note:* If deciding to use a `uniform`, then the size of the array needs to be defined at compile time.
This can be achieved with a shader variant definition.*
...
![loadcycles](../../../_images/samples/performance/constant_data/images/loadcycles.png)
...
* 
Vulkan Fast Paths [https://gpuopen.com/wp-content/uploads/2016/03/VulkanFastPaths.pdf](https://gpuopen.com/wp-content/uploads/2016/03/VulkanFastPaths.pdf)
