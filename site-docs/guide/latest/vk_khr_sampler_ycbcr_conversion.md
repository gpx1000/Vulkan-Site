...
- **Component**: guide
- **Version**: latest
- **URL**: /guide/latest/extensions/VK_KHR_sampler_ycbcr_conversion.html

## Table of Contents

- [Multi-planar Formats](#multi-planar-formats)
- [Disjoint](#_disjoint)
- [Copying memory to each plane](#_copying_memory_to_each_plane)
- [Copying_memory_to_each_plane](#_copying_memory_to_each_plane)
- [VkSamplerYcbcrConversion](#_vksamplerycbcrconversion)
- [combinedImageSamplerDescriptorCount](#_combinedimagesamplerdescriptorcount)
...
It is worth noting here is that the `imageOffset` is zero because its base is the plane, not the entire `VkImage`. So when using the `imageOffset` make sure to start from base of the plane and not always plane 0.

The `VkSamplerYcbcrConversion` describes all the “out of scope explaining here” aspects of Y′CBCR conversion which are described in the [Khronos Data Format Specification](https://registry.khronos.org/DataFormat/specs/1.3/dataformat.1.3.html#_introduction_to_color_conversions). The values set here are dependent on the input Y′CBCR data being obtained and how to do the conversion to RGB color spacce.
