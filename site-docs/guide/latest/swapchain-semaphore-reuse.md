...
- **Component**: guide
- **Version**: latest
- **URL**: /guide/latest/swapchain_semaphore_reuse.html

## Table of Contents

- [Problem Statement](#_problem_statement)
- [Discussion of Solution](#_discussion_of_solution)
- [Discussion_of_Solution](#_discussion_of_solution)
- [VK_EXT_swapchain_maintenance1 extension](#_vk_ext_swapchain_maintenance1_extension)
- [VK_EXT_swapchain_maintenance1_extension](#_vk_ext_swapchain_maintenance1_extension)
...
While fixing simpler apps like `vkcube`, applying the fix really was as straightforward as described. Of course, for complex engine setups it can be a different experience. [Here’s an example](https://github.com/kennyalive/vulkan-base/commit/27bcaad9d519cc2f9c5cde4872742d4a5212eee6) of what the fix looks like in the "hello-world" style Vulkan application.
...
The purpose of the above code is to explain how to handle swapchain wait semaphores without additional extensions, although implementations that support the [VK_EXT_swapchain_maintenance1](https://registry.khronos.org/vulkan/specs/latest/man/html/VK_EXT_swapchain_maintenance1.html) extension do provide an alternative solution. This extension makes `vkQueuePresentKHR` more similar to `vkQueueSubmit`, allowing it to specify a fence that the application can wait on.
