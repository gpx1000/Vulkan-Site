...
Of course, just like dynamic memory allocation in C++, the memory should be freed at some point.
...
This is done by [mapping the buffer memory](https://en.wikipedia.org/wiki/Memory-mapped_I/O) into CPU accessible memory with `vkMapMemory`.
...
The transfer of data to the GPU is an operation that happens in the background and the specification simply [tells us](https://www.khronos.org/registry/vulkan/specs/1.3-extensions/html/chap7.html#synchronization-submission-host-writes) that it is guaranteed to be complete as of the next call to `vkQueueSubmit`.
...
![triangle white](../_images/images/triangle_white.png)
...
[C++ code](../_attachments/19_vertex_buffer.cpp) / [Vertex shader](../_attachments/18_shader_vertexbuffer.vert) / [Fragment shader](../_attachments/18_shader_vertexbuffer.frag)
