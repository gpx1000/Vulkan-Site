...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/09_Generating_Mipmaps.html

## Table of Contents

- [Introduction](#_introduction)
- [Image creation](#_image_creation)
- [Generating Mipmaps](#_generating_mipmaps)
- [Linear filtering support](#_linear_filtering_support)
- [Linear_filtering_support](#_linear_filtering_support)
- [Sampler](#_sampler)
...
Using smaller images increases the rendering speed and avoids artifacts such as [Moiré patterns](https://en.wikipedia.org/wiki/Moir%C3%A9_pattern).
An example of what mipmaps look like:

![mipmaps example](_images/images/mipmaps_example.jpg)

In Vulkan, each of the mip images is stored in different *mip levels* of a `vk::Image`.
...
...
transitionImageLayout(commandBuffer, textureImage, vk::ImageLayout::eUndefined, vk::ImageLayout::eTransferDstOptimal, mipLevels);
copyBufferToImage(commandBuffer, stagingBuffer, textureImage, static_cast(texWidth), static_cast(texHeight));
...
Beware if you are using a dedicated transfer queue (as suggested in [Vertex buffers](04_Vertex_buffers/02_Staging_buffer.html)): `vk::raii::CommandBuffer::blitImage` must be submitted to a queue with graphics capability.
...
![highmipmaps](_images/images/highmipmaps.png)

This is how higher mip levels will be used when objects are further away from the camera.

The [next chapter](10_Multisampling.html) will walk us through multisampling to produce a smoother image.

[C++ code](_attachments/29_mipmapping.cpp) /
