...
Beware if you are using a dedicated transfer queue (as suggested in [Vertex buffers](04_Vertex_buffers/02_Staging_buffer.html)): `vkCmdBlitImage` must be submitted to a queue with graphics capability.
...
[C++ code](_attachments/29_mipmapping.cpp) / [Vertex shader](_attachments/27_shader_depth.vert) / [Fragment shader](_attachments/27_shader_depth.frag)
