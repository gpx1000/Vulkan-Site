...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/14_Android.html

## Table of Contents

- [Introduction](#_introduction)
- [Android-specific Considerations](#_android_specific_considerations)
- [Project Setup](#_project_setup)
- [Prerequisites](#_prerequisites)
- [Project Structure](#_project_structure)
- [Setting Up the Android Project](#_setting_up_the_android_project)
- [Setting_Up_the_Android_Project](#_setting_up_the_android_project)
- [The Manifest File](#_the_manifest_file)
- [The_Manifest_File](#_the_manifest_file)
- [Java Activity](#_java_activity)
- [Build Configuration](#_build_configuration)
- [CMake Configuration](#_cmake_configuration)
- [Native Implementation](#_native_implementation)
- [GameActivity Bridge](#_gameactivity_bridge)
- [Android Entry Point](#_android_entry_point)
- [Android_Entry_Point](#_android_entry_point)
- [Creating the Vulkan Surface](#_creating_the_vulkan_surface)
- [Creating_the_Vulkan_Surface](#_creating_the_vulkan_surface)
- [Handling Android Events](#_handling_android_events)
- [Handling_Android_Events](#_handling_android_events)
- [Cross-Platform Implementation](#_cross_platform_implementation)
- [Platform Detection](#_platform_detection)
- [Consistent Class Structure](#_consistent_class_structure)
- [Consistent_Class_Structure](#_consistent_class_structure)
- [Platform-Specific Includes](#_platform_specific_includes)
- [Cross-Platform File Loading](#_cross_platform_file_loading)
- [Cross-Platform_File_Loading](#_cross_platform_file_loading)
- [Platform-Specific Entry Points](#_platform_specific_entry_points)
- [Platform-Specific_Entry_Points](#_platform_specific_entry_points)
- [Build System Integration](#_build_system_integration)
- [Build_System_Integration](#_build_system_integration)
- [Shader Handling on Android](#_shader_handling_on_android)
- [Shader_Handling_on_Android](#_shader_handling_on_android)
- [Local Shader Compilation](#_local_shader_compilation)
- [Local_Shader_Compilation](#_local_shader_compilation)
- [Loading Assets in a Cross-Platform Way](#_loading_assets_in_a_cross_platform_way)
- [Loading_Assets_in_a_Cross-Platform_Way](#_loading_assets_in_a_cross_platform_way)
- [Building and Running](#_building_and_running)
- [Building_and_Running](#_building_and_running)
...
* 
**[Android Studio](https://developer.android.com/studio[Android Studio])**: The official IDE for Android development
...
|  | Unlike the desktop environment, Vulkan HPP (the C++ bindings for Vulkan) is NOT included by default in the Android NDK. You’ll need to download it separately from the [Vulkan-Hpp GitHub repository](https://github.com/KhronosGroup/Vulkan-Hpp) or use the version included in the Vulkan SDK. |
| --- | --- |
...
Creates an android_app structure compatible with our Vulkan code

Sets up the necessary connections between GameActivity and our code

Calls the android_main function in our 34_android.cpp file
...
static void handleAppCommand(android_app* app, int32_t cmd) {
    auto* vulkanApp = static_cast(app->userData);
    switch (cmd) {
...
This approach leverages the standard predefined macro `*ANDROID*` which is automatically defined by the compiler when building for Android platforms. These platform macros are then used throughout the code to conditionally compile platform-specific code.
...
std::vector readFile(const std::string& filename, std::optional assetManager = std::nullopt) {
#if PLATFORM_ANDROID
...
Our cross-platform approach leverages the compiler’s built-in platform detection capabilities. Since the `*ANDROID*` macro is automatically defined by the compiler when building for Android, we don’t need to explicitly define platform macros in our build system.

This approach has several advantages:

**Simplicity**: We don’t need to maintain platform-specific compile definitions in our CMake files.

**Reliability**: We rely on standard compiler behavior rather than custom definitions.

**Maintainability**: Less build system configuration means fewer potential points of failure.
...
#if PLATFORM_ANDROID
std::optional optionalAssetManager = assetManager;
#else
std::optional optionalAssetManager = std::nullopt;
#endif
std::vector vertShaderCode = readFile("shaders/vert.spv", optionalAssetManager);
std::vector fragShaderCode = readFile("shaders/frag.spv", optionalAssetManager);
...
#if PLATFORM_ANDROID
std::optional optionalAssetManager = assetManager;
std::vector imageData = readFile(TEXTURE_PATH, optionalAssetManager);
