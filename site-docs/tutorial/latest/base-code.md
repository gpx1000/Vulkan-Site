...
In C++ it is possible to perform automatic resource  management using [RAII](https://en.wikipedia.org/wiki/Resource_Acquisition_Is_Initialization)  or smart pointers provided in the `` header.
...
First replace the `#include ` line with
...
Now that we have the skeleton for the Vulkan application, let’s [create the first Vulkan object](01_Instance.html)!
