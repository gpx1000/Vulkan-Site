...
- **Component**: tutorial
- **Version**: latest
- **URL**: /tutorial/latest/03_Drawing_a_triangle/02_Graphics_pipeline_basics/01_Shader_modules.html

## Table of Contents

- [Vertex shader](#_vertex_shader)
- [Fragment shader](#_fragment_shader)
- [Per-vertex colors](#_per_vertex_colors)
- [Compiling the shaders](#_compiling_the_shaders)
- [Compiling_the_shaders](#_compiling_the_shaders)
- [Loading a shader](#_loading_a_shader)
- [Loading_a_shader](#_loading_a_shader)
- [Creating shader modules](#_creating_shader_modules)
- [Creating_shader_modules](#_creating_shader_modules)
- [Shader stage creation](#_shader_stage_creation)
- [Shader_stage_creation](#_shader_stage_creation)

## Content

Unlike earlier APIs, shader code in Vulkan has to be specified in a bytecode format as opposed to human-readable syntax like [GLSL](https://registry.khronos.org/OpenGL/specs/gl/GLSLangSpec.4.60.html) and [HLSL](https://en.wikipedia.org/wiki/High-Level_Shading_Language).
This bytecode format is called [SPIR-V](https://www.khronos.org/spir) and is designed to be used with both Vulkan and OpenCL (both Khronos APIs).
...
The advantage of `glslc` is that it uses the same parameter format as well-known compilers like GCC and Clang and includes some extra functionality like *includes*.
...
A *clip coordinate* is a four dimensional vector from the vertex shader that is subsequently turned into a *normalized device coordinate* by dividing the whole vector by its last component.
These normalized device coordinates are [homogeneous coordinates](https://en.wikipedia.org/wiki/Homogeneous_coordinates) that map the framebuffer to a [-1, 1] by [-1, 1] coordinate system that looks like the following:

![normalized device coordinates](../../_images/images/normalized_device_coordinates.svg)
...
![triangle coordinates](../../_images/images/triangle_coordinates.svg)
...
![triangle coordinates colors](../../_images/images/triangle_coordinates_colors.png)
...
The Vulkan SDK includes [libshaderc](https://github.com/google/shaderc), which is a library to compile GLSL code to SPIR-V from within your program.
...
In the [next chapter](02_Fixed_functions.html) we’ll look at the fixed-function stages.

[C++ code](../../_attachments/09_shader_modules.cpp) / [Vertex shader](../../_attachments/09_shader_base.vert) / [Fragment shader](../../_attachments/09_shader_base.frag)
