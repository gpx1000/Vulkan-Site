...
If `pDepthStencilResolveAttachment` is not `NULL` and does not have
the value `VK_ATTACHMENT_UNUSED` and [VkFormat](VkFormat.html) of
...
If `pDepthStencilResolveAttachment` is not `NULL` and does not have
the value `VK_ATTACHMENT_UNUSED` and the [VkFormat](VkFormat.html) of
`pDepthStencilResolveAttachment` has a depth component, then the
...
If `pDepthStencilResolveAttachment` is not `NULL` and does not have
the value `VK_ATTACHMENT_UNUSED`, the [VkFormat](VkFormat.html) of
...
`VK_ATTACHMENT_UNUSED`, and has a [VkFormat](VkFormat.html) that has a depth
...
`multisampledRenderToSingleSampledEnable` field is `VK_TRUE`,
and `pDepthStencilAttachment` is not `NULL`, does not have the value
`VK_ATTACHMENT_UNUSED`, and has a [VkFormat](VkFormat.html) with a stencil
...
`VK_ATTACHMENT_UNUSED`, and has a [VkFormat](VkFormat.html) with both depth and
